...
namespace DC2D
{
	public class SimplexNoise
	{
		...
		public static float Noise(float x, float y, float z, float w)
		{
			...
			float G4 = (float)(5.0f - Math.Sqrt(5.0f)) / 20.0f;
		...
		public static float SeamlessNoise(float x, float y, float dx, float dy, float xyOffset, int octaves, float pers)
		{
			...
			{
				float nx = xyOffset + (float)Math.Cos(x * 2.0f * Math.PI) * dx / (float)(2.0f * Math.PI);
				float ny = xyOffset + (float)Math.Cos(y * 2.0f * Math.PI) * dy / (float)(2.0f * Math.PI);
				float nz = xyOffset + (float)Math.Sin(x * 2.0f * Math.PI) * dx / (float)(2.0f * Math.PI);
				float nw = xyOffset + (float)Math.Sin(y * 2.0f * Math.PI) * dy / (float)(2.0f * Math.PI);

				noise += Noise(nx, ny, nz, nw) * amp;
